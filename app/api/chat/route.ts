...
export async function POST(req: Request) {
  if (!process.env.OPENAI_API_KEY) {
    return Response.json(
      { text: "System Error: OpenAI API Key is missing. Please configure it in your environment variables." },
      { status: 500 }
    );
  }
  ...
  try {
    ...
    return Response.json(
      { text: "The AI service is currently unavailable. Please check the API quota or try again later." },
      { status: 500 }
    );
  }
}
